...
class WorkoutsRepository {
  ...
  Future<List<Workout>> listPublic() => _cachedList('$_base/public', _publicKey);
  ...
  Future<List<Workout>> _cachedList(String url, String key) async {
    try {
      ...
      if (resp.statusCode != 200) throw Exception('GET $url HTTP ${resp.statusCode}');
      ...
      if (_isOffline(e) && _store.hasList(key)) {
        return _store.getListDocs(_collection, key).map(Workout.fromJson).toList();
      }
      rethrow;
    }
  }
  ...
  Future<Workout> create({
    ...
    final doc = _localWorkoutDoc(id: tempId, name: name, comment: comment, exercises: exercises);
    ...
    if (await ConnectivityService.instance.isOnline()) {
      try {
        return await _networkCreate(name, comment, serverExercises, replaceTempId: tempId);
      } on Object catch (e) {
        if (!_isOffline(e)) {
    ...
    await _enqueue('workout.create', {'tempId': tempId, 'name': name, 'comment': comment, 'exercises': serverExercises});
    return Workout.fromJson(doc);
  }
  ...
  Future<Workout> update(
    ...
    final serverExercises = exercises.map((e) => e.toJson()).toList();
    final doc = _localWorkoutDoc(id: id, name: name, comment: comment, exercises: exercises, base: _store.getDoc(_collection, id));
    await _store.putDoc(_collection, id, doc);

    if (!id.startsWith('local:') && await ConnectivityService.instance.isOnline()) {
      try {
        final resp = await _client
            .put(Uri.parse('$_base/$id'), body: _encode(name, comment, serverExercises))
    ...
    await _enqueue('workout.update', {'id': id, 'name': name, 'comment': comment, 'exercises': serverExercises});
    return Workout.fromJson(doc);
  }
  ...
  Future<Workout> _networkCreate(String name, String comment, List<Map<String, dynamic>> exercises,
      {required String replaceTempId}) async {
    final resp = await _client
        .post(Uri.parse(_base), body: _encode(name, comment, exercises))
        .timeout(const Duration(seconds: 20));
    if (resp.statusCode != 201) throw Exception(_err(resp.body, resp.statusCode));
    final fresh = jsonDecode(resp.body) as Map<String, dynamic>;
    await _store.remapId(_collection, replaceTempId, fresh['id'] as String, fresh);
    return Workout.fromJson(fresh);
  }

  Future<void> _enqueue(String kind, Map<String, dynamic> args) async {
    await _store.enqueue(Mutation(id: _uuid.v4(), seq: _store.nextSeq(), kind: kind, args: args));
  ...
  void _registerHandlers() {
      ...
      try {
        ...
        if (resp.statusCode == 201) {
          ...
          return SyncOutcome.done(
              collection: _collection, remapFromId: m.args['tempId'] as String, remapToId: fresh['id'] as String, realDoc: fresh);
        }
        return _isClientError(resp.statusCode) ? const SyncOutcome.drop() : const SyncOutcome.retry();
      } on Object catch (e) {
        return _isOffline(e) ? const SyncOutcome.retry() : const SyncOutcome.drop();
      }
    });

    s.registerHandler('workout.update', (client, m) async {
      final id = m.args['id'] as String;
      if (id.startsWith('local:')) return const SyncOutcome.retry(); // create hasn't synced yet
      return _replay(() => client
          .put(Uri.parse('$base/$id'), body: _encodeArgs(m.args))
          .timeout(const Duration(seconds: 20)), ok: 200);
    });

    s.registerHandler('workout.delete', (client, m) async {
      final id = m.args['id'] as String;
      return _replay(() => client.delete(Uri.parse('$base/$id')).timeout(const Duration(seconds: 15)), ok: 204);
    });
    ...
    s.registerHandler('workout.run', (client, m) async {
      ...
      return _replay(() => client
          .post(Uri.parse('$base/$id/run'), body: jsonEncode({'difficulty': m.args['difficulty']}))
          .timeout(const Duration(seconds: 15)), ok: 204);
    });
    ...
    s.registerHandler('workout.visibility', (client, m) async {
      ...
      if (id.startsWith('local:')) return const SyncOutcome.retry();
      return _replay(() => client
          .put(Uri.parse('$base/$id/visibility'), body: jsonEncode({'visibility': m.args['visibility']}))
          .timeout(const Duration(seconds: 15)), ok: 204);
    });
  }

  static Future<SyncOutcome> _replay(Future<http.Response> Function() call, {required int ok}) async {
    try {
  ...
  static String _encode(String name, String comment, List<Map<String, dynamic>> exercises) =>
  ...
  static bool _isOffline(Object e) => e is TimeoutException || e is http.ClientException;
