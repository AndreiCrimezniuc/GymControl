...
enum MovePattern {
  squat, hinge, horizontalPress, overheadPress, curl, tricepsExtension,
  row, pulldown, lateralRaise, calfRaise, crunch, generic,
}

/// Picks a movement pattern from an exercise's name / muscle / equipment.
MovePattern patternFor({required String name, required String muscle, required String equipment}) {
  final n = name.toLowerCase();
  final m = muscle.toLowerCase();
  bool inName(List<String> k) => k.any(n.contains);

  if (inName(['deadlift', 'romanian', 'good morning', 'hip thrust', 'swing', 'hyperextension', 'rdl'])) return MovePattern.hinge;
  if (inName(['squat', 'lunge', 'leg press', 'split squat', 'step up', 'pistol'])) return MovePattern.squat;
  if (inName(['bench press', 'push-up', 'pushup', 'push up', 'chest press', 'dips', 'dip', 'fly', 'flye'])) return MovePattern.horizontalPress;
  if (inName(['overhead press', 'shoulder press', 'military', 'ohp', 'push press', 'arnold'])) return MovePattern.overheadPress;
  if (inName(['lateral raise', 'front raise', 'side raise', 'reverse fly', 'lateral'])) return MovePattern.lateralRaise;
  if (inName(['curl'])) return MovePattern.curl;
  if (inName(['pushdown', 'triceps extension', 'skullcrusher', 'kickback', 'overhead extension', 'triceps'])) return MovePattern.tricepsExtension;
  if (inName(['pulldown', 'pull-up', 'pullup', 'pull up', 'chin-up', 'chin up'])) return MovePattern.pulldown;
  if (inName(['row'])) return MovePattern.row;
  if (inName(['calf', 'calve'])) return MovePattern.calfRaise;
  if (inName(['crunch', 'sit-up', 'situp', 'leg raise', 'plank', 'ab '])) return MovePattern.crunch;

  // fall back on the muscle group
  if (m.contains('quad') || m.contains('glute') || m.contains('hamstring') || m.contains('leg')) return MovePattern.squat;
  if (m.contains('chest') || m.contains('pec')) return MovePattern.horizontalPress;
  if (m.contains('shoulder') || m.contains('delt')) return MovePattern.overheadPress;
...
class _Pose {
  ...
  const _Pose({this.torso = -90, this.neck = -90, this.upperArm = 90, this.foreArm = 90, this.thigh = 90, this.shin = 90});
...
_Move _moveFor(MovePattern p) {
  switch (p) {
    case MovePattern.squat:
      return const _Move(
        _Pose(torso: -82, neck: -86, upperArm: 2, foreArm: -6, thigh: 90, shin: 90),
        _Pose(torso: -62, neck: -78, upperArm: -2, foreArm: -6, thigh: 25, shin: 118),
        {'thigh', 'shin'},
      );
    case MovePattern.hinge:
      return const _Move(
        _Pose(torso: -85, neck: -86, upperArm: 90, foreArm: 90, thigh: 88, shin: 90),
        _Pose(torso: -22, neck: -28, upperArm: 90, foreArm: 90, thigh: 72, shin: 96),
        {'torso', 'thigh'},
      );
    case MovePattern.horizontalPress:
      // profile "plank press" proxy: near-horizontal body, elbows bend
      return const _Move(
        _Pose(torso: -12, neck: -20, upperArm: 90, foreArm: 90, thigh: 172, shin: 176),
        _Pose(torso: -12, neck: -20, upperArm: 66, foreArm: 112, thigh: 172, shin: 176),
        {'upperArm', 'foreArm'},
      );
    case MovePattern.overheadPress:
      return const _Move(
        _Pose(torso: -90, neck: -90, upperArm: -58, foreArm: -118, thigh: 90, shin: 90),
        _Pose(torso: -90, neck: -90, upperArm: -90, foreArm: -90, thigh: 90, shin: 90),
        {'upperArm', 'foreArm'},
      );
    case MovePattern.curl:
      return const _Move(
        _Pose(torso: -90, neck: -90, upperArm: 88, foreArm: 92, thigh: 90, shin: 90),
        _Pose(torso: -90, neck: -90, upperArm: 88, foreArm: -62, thigh: 90, shin: 90),
        {'foreArm'},
      );
    case MovePattern.tricepsExtension:
      return const _Move(
        _Pose(torso: -90, neck: -90, upperArm: 92, foreArm: 32, thigh: 90, shin: 90),
        _Pose(torso: -90, neck: -90, upperArm: 92, foreArm: 92, thigh: 90, shin: 90),
        {'foreArm'},
      );
    case MovePattern.row:
      return const _Move(
        _Pose(torso: -28, neck: -34, upperArm: 96, foreArm: 96, thigh: 82, shin: 92),
        _Pose(torso: -28, neck: -34, upperArm: 140, foreArm: 96, thigh: 82, shin: 92),
        {'upperArm', 'torso'},
      );
    case MovePattern.pulldown:
      return const _Move(
        _Pose(torso: -92, neck: -92, upperArm: -74, foreArm: -80, thigh: 90, shin: 90),
        _Pose(torso: -96, neck: -96, upperArm: -26, foreArm: 34, thigh: 90, shin: 90),
        {'upperArm'},
      );
    case MovePattern.lateralRaise:
      return const _Move(
        _Pose(torso: -90, neck: -90, upperArm: 88, foreArm: 88, thigh: 90, shin: 90),
        _Pose(torso: -90, neck: -90, upperArm: 3, foreArm: 1, thigh: 90, shin: 90),
        {'upperArm'},
      );
    case MovePattern.calfRaise:
      return const _Move(
        _Pose(torso: -90, neck: -90, upperArm: 88, foreArm: 90, thigh: 90, shin: 90),
        _Pose(torso: -90, neck: -90, upperArm: 88, foreArm: 90, thigh: 90, shin: 84),
        {'shin'},
      );
    case MovePattern.crunch:
      return const _Move(
        _Pose(torso: -78, neck: -78, upperArm: -40, foreArm: -70, thigh: 44, shin: 128),
        _Pose(torso: -52, neck: -52, upperArm: -34, foreArm: -64, thigh: 44, shin: 128),
        {'torso'},
      );
    case MovePattern.generic:
      return const _Move(
        _Pose(torso: -90, neck: -90, upperArm: 86, foreArm: 89, thigh: 90, shin: 90),
        _Pose(torso: -90, neck: -90, upperArm: 84, foreArm: 88, thigh: 84, shin: 94),
        {},
      );
  }
}
...
class ExerciseMannequin extends StatefulWidget {
  ...
  final bool animate;
  const ExerciseMannequin({super.key, required this.pattern, this.animate = false});

  @override
  State<ExerciseMannequin> createState() => _ExerciseMannequinState();
}

class _ExerciseMannequinState extends State<ExerciseMannequin> with SingleTickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    ...
    if (_ctrl == null) {
      return SizedBox.expand(
        child: CustomPaint(painter: _MannequinPainter(move: move, colors: c, t: 0.5)),
      );
    }
    return AnimatedBuilder(
      ...
      builder: (_, __) {
        ...
        return SizedBox.expand(child: CustomPaint(painter: _MannequinPainter(move: move, colors: c, t: t)));
      },
    );
  }
}
...
const double _torsoLen = 26, _neckLen = 6, _headR = 6.5, _upperArm = 16, _foreArm = 15, _thigh = 20, _shin = 20, _footLen = 9;
...
class _MannequinPainter extends CustomPainter {
  ...
  _MannequinPainter({required this.move, required this.colors, required this.t});
  ...
  Map<String, Offset> _joints(_Pose p) {
    ...
    return {
      'hip': hip, 'shoulder': shoulder, 'head': head, 'elbow': elbow,
      'wrist': wrist, 'knee': knee, 'ankle': ankle, 'toe': toe,
    };
  }
  ...
  void paint(Canvas canvas, Size size) {
    ...
    const pad = 10.0;
    final scale = math.min((size.width - pad * 2) / bboxW, (size.height - pad * 2) / bboxH);
